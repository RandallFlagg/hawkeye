...
namespace Acorns.Hawkeye.DynamicExtender.Snapshot
{
    /// <summary>
    /// This extender adds a "Snapshot" button allowing to capture an image of the currently selected control.
    /// </summary>
    public class SnapshotExtender : IDynamicSubclass
    {
        private IHawkeyeEditor editor = null;
        private ToolBarButton button = null;

        /// <summary>
        /// Initializes a new instance of the <see cref="SnapshotExtender"/> class.
        /// </summary>
        public SnapshotExtender()
        {
            Trace.WriteLine("SnapshotExtender: Loaded.");
        }
        ...
        public void Attach(object target)
        {
            if (target == null)
            {
                Trace.WriteLine("SnapshotExtender: Can't attach: target is null.");
                return;
            }

            if (!(target is IHawkeyeEditor))
            {
                Trace.WriteLine("SnapshotExtender: Can't attach: target is not a IHawkeyeEditor.");
                return;
            }
